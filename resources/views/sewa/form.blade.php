...
<head>
    ...
    <title>Sewa {{ $alat->merk }}</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="bg-gray-100">

<div class="grid md:grid-cols-3 gap-8">  <!-- dari cols-2 jadi cols-3 -->
    <!-- Kolom FOTO lebih kecil -->
    <div class="md:col-span-1">  <!-- kolom foto cuma 1/3 -->
        ...
        <div class="mt-6 text-center">
            <p class="text-4xl font-bold text-yellow-600">
                Rp {{ number_format($alat->harga_sewa) }} <span class="text-lg font-normal text-gray-600">/ hari</span>
            </p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8">
            ...
            <form action="{{ route('pesan.store') }}" method="POST" id="formSewa">
                @csrf
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" name="nama" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-yellow-500" placeholder="John Doe">
                    </div>

                    <div>
                        <label class="block font-medium mb-1">No. HP / WhatsApp</label>
                        <input type="text" name="no_hp" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-yellow-500" placeholder="081234567890">
                    </div>

                    <div>
                        <label class="block font-medium mb-1">Tanggal Mulai</label>
                        <input type="date" name="tgl_mulai" id="mulai" required min="{{ date('Y-m-d') }}" class="w-full px-4 py-3 border rounded-lg">
                    </div>

                    <div>
                        <label class="block font-medium mb-1">Tanggal Selesai</label>
                        <input type="date" name="tgl_sewa" id="selesai" required class="w-full px-4 py-3 border rounded-lg">
                    </div>
                    ...
                    <button type="submit" class="w-full bg-yellow-500 hover:bg-yellow-600 text-white font-bold text-xl py-4 rounded-lg transition">
                        PESAN SEKARANG
                    </button>
                </div>
            </form>
